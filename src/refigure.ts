...
function format(nstr: string, isNegative: boolean, prefix: string): string {
  let dp = nstr.length;
...
export default function refigure(value: BigNumber): string {
  if (value.isZero()) {
    ...
    if (magnitude.isGreaterThanOrEqualTo(MAX_MAGNITUDE_EXCLUSIVE)) {
      ...
      return format(
        magnitude.shiftedBy(-groups * 3).toFixed(),
        isNegative,
        PREFIX_HIGH[groups],
      );
    } else {
      ...
      return format(
        magnitude.shiftedBy(groups * 3).toFixed(dp, BigNumber.ROUND_HALF_EVEN),
        isNegative,
        PREFIX_LOW[groups],
      );
    }
